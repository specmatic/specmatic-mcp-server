...
**IMPORTANT:** To enable JUnit XML report generation and analysis, configure your MCP client so the server can write reports to a host-mounted `reports/` directory.

### Recommended setup
- Prefer the npm package for local development and testing.
- If you run the MCP server in Docker, mount `./reports` from the host to `/app/reports` in the container.
- Keep client-specific setup commands in user-facing docs such as `README.md`, not in this shared agent file.
...
- Your coding agent to read detailed test reports for analysis
